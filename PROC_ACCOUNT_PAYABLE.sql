drop procedure if Exists PROC_ACCOUNT_PAYABLE;
DELIMITER $$
CREATE PROCEDURE `PROC_ACCOUNT_PAYABLE`( P_VENDOR_ID TEXT,
...
BEGIN
	...
	SET @QRY = CONCAT(' SELECT CASE
					      ...
					      SELECT B.VENDOR_ID,
								...
								SELECT B.VENDOR_ID,
									   C.RECEIVE_ENTRY_DATE AS ENTRY_DATE,
									   DATE_ADD(C.RECEIVE_ENTRY_DATE, INTERVAL IF(B.DEFAULT_NUMBER_OF_DAYS > 0, B.DEFAULT_NUMBER_OF_DAYS - 1, B.DEFAULT_NUMBER_OF_DAYS) DAY) AS DUE_DATE,
									   C.RO_ID AS FORM_ID,
									   C.PAYPAL_TRANSACTION_ID,
									   C.RECEIVE_TOTAL_AMOUNT AS AMOUNT,
									   (A.FORM_AMOUNT) AS FORM_AMOUNT,
									   D.TERM_NAME,
									   B.DEFAULT_NUMBER_OF_DAYS,
									   DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) AS AGE,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) >= 0 AND DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) <= 30 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_1,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) >= 31 AND DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) <= 60 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_2,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) >= 61 AND DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) <= 90 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_3,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.RECEIVE_ENTRY_DATE) >= 91 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_4,
                                       ''Receive Order'' AS FORM_NAME
								  FROM ((((SELECT VENDOR_ID,
												  FORM_ID,
                                                  FORM_FLAG,
                                                  FORM_AMOUNT
									   ...
									   JOIN VENDOR B ON (A.VENDOR_ID = B.ID 
														 AND CASE
																  ...
																  ELSE TRUE
															 END))
									   JOIN RECEIVE_ORDER C ON (A.FORM_ID = C.ID 
															   AND A.FORM_FLAG = ''R''
															   AND C.COMPANY_ID = \'',P_COMPANY_ID,'\'
															   AND CASE
																		WHEN \'',P_VENDOR_ID,'\' <> "" THEN C.VENDOR_ID = \'',P_VENDOR_ID,'\'
																   ...
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_FROM,'\' <> "" THEN C.RECEIVE_ENTRY_DATE >= \'',P_ENTRY_DATE_FROM,'\'
																		ELSE TRUE
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_TO,'\' <> "" THEN C.RECEIVE_ENTRY_DATE <= \'',P_ENTRY_DATE_TO,'\'
																		ELSE TRUE
																   END))
									   JOIN PAYMENT_TERMS D ON (B.PAYMENT_TERM_ID = D.ID))
								...
								SELECT B.VENDOR_ID,
									   C.VCM_ENTRY_DATE AS ENTRY_DATE,
									   DATE_ADD(C.VCM_ENTRY_DATE, INTERVAL B.DEFAULT_NUMBER_OF_DAYS DAY) AS DUE_DATE,
									   C.VCM_ID AS FORM_ID,
									   C.PAYPAL_TRANSACTION_ID,
									   C.VCM_TOTAL AS AMOUNT,
									   (A.FORM_AMOUNT) AS FORM_AMOUNT,
									   D.TERM_NAME,
									   B.DEFAULT_NUMBER_OF_DAYS,
									   DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) AS AGE,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) >= 0 AND DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) <= 30 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_1,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) >= 31 AND DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) <= 60 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_2,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) >= 61 AND DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) <= 90 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_3,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.VCM_ENTRY_DATE) >= 91 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_4,
                                       ''Vendor Credit Memo'' AS FORM_NAME
								  FROM ((((SELECT VENDOR_ID,
												  FORM_ID,
                                                  FORM_FLAG,
                                                  FORM_AMOUNT
									   ...
									   JOIN VENDOR B ON (A.VENDOR_ID = B.ID 
														 AND CASE
																  ...
																  ELSE TRUE
															 END))
									   JOIN VENDOR_CREDIT_MEMO C ON (A.FORM_ID = C.ID 
															   AND A.FORM_FLAG = ''V''
															   AND C.COMPANY_ID = \'',P_COMPANY_ID,'\'
															   AND CASE
																		WHEN \'',P_VENDOR_ID,'\' <> "" THEN C.VENDOR_ID = \'',P_VENDOR_ID,'\'
																   ...
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_FROM,'\' <> "" THEN C.VCM_ENTRY_DATE >= \'',P_ENTRY_DATE_FROM,'\'
																		ELSE TRUE
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_TO,'\' <> "" THEN C.VCM_ENTRY_DATE <= \'',P_ENTRY_DATE_TO,'\'
																		ELSE TRUE
																   END))
									   JOIN PAYMENT_TERMS D ON (B.PAYMENT_TERM_ID = D.ID))
								...
								SELECT B.VENDOR_ID,
									   C.SN_ENTRY_DATE AS ENTRY_DATE,
									   DATE_ADD(C.SN_ENTRY_DATE, INTERVAL B.DEFAULT_NUMBER_OF_DAYS DAY) AS DUE_DATE,
									   C.SN_ID AS FORM_ID,
									   C.SN_ID AS PAYPAL_TRANSACTION_ID,
									   C.TOTAL_AMOUNT_IN AS AMOUNT,
									   (A.FORM_AMOUNT) AS FORM_AMOUNT,
									   D.TERM_NAME,
									   B.DEFAULT_NUMBER_OF_DAYS,
									   DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) AS AGE,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) >= 0 AND DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) <= 30 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_1,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) >= 31 AND DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) <= 60 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_2,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) >= 61 AND DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) <= 90 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_3,
									   CASE 
											WHEN DATEDIFF(CURDATE(), C.SN_ENTRY_DATE) >= 91 THEN (A.FORM_AMOUNT)
											ELSE NULL
									   END AS RANGE_4,
                                       ''Stock In'' AS FORM_NAME
								  FROM ((((SELECT VENDOR_ID,
												  FORM_ID,
                                                  FORM_FLAG,
                                                  FORM_AMOUNT
									   ...
									   JOIN VENDOR B ON (A.VENDOR_ID = B.ID 
														 AND CASE
																  ...
																  ELSE TRUE
															 END))
									   JOIN VW_STOCK_IN C ON (A.FORM_ID = C.ID 
															   AND A.FORM_FLAG = ''N''
															   AND C.COMPANY_ID = \'',P_COMPANY_ID,'\'
															   AND CASE
																		WHEN \'',P_VENDOR_ID,'\' <> "" THEN C.VENDOR_ID = \'',P_VENDOR_ID,'\'
																   ...
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_FROM,'\' <> "" THEN C.SN_ENTRY_DATE >= \'',P_ENTRY_DATE_FROM,'\'
																		ELSE TRUE
																   END
															   AND CASE
																		WHEN \'',P_ENTRY_DATE_TO,'\' <> "" THEN C.SN_ENTRY_DATE <= \'',P_ENTRY_DATE_TO,'\'
																		ELSE TRUE
																   END))
									   JOIN PAYMENT_TERMS D ON (B.PAYMENT_TERM_ID = D.ID))
                      ...
                      GROUP BY E.VENDOR_ID, E.FORM_ID, E.FORM_NAME, E.ENTRY_DATE, E.DUE_DATE, E.PAYPAL_TRANSACTION_ID, E.TERM_NAME, E.AGE, E.DEFAULT_NUMBER_OF_DAYS WITH ROLLUP
